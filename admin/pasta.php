...
<?php
$conn = connect();
$good_data = selectRows($conn, "SELECT * FROM pasta_price");
$good_count = selectOneRow($conn, "SELECT COUNT(*) FROM pasta_price");
close($conn);
?>
<div class="admin-wrapper">
	<div class="title-department">Макарон</div>
			...
			<div class="block-content d-flex justify-content-between pt-2">
				<div class="main-container">
                    ...
                    <?php
                        ...
                        for ($i=0; $i < count($good_data); $i++) { 
                            $out .= '       
								...
								<div class="item-good d-flex align-items-center">
									<a href="#" data-good-id="'.$good_data[$i]['id'].'" data-category="pasta_price" data-image1="'.$good_data[$i]['image_1'].'" data-image2="'.$good_data[$i]['image_2'].'" id="delete_good">
									...
									<img src="../img/pasta/'.$good_data[$i]['image_1'].'" alt="">
									<p class="m-0 p-0">'.$good_data[$i]['name'].'</p>
									<div class="d-flex price-block">
										<span>'.$good_data[$i]['cost'].'</span>
										<a href="edit_good.php?good_id='.$good_data[$i]['id'].'&good_category=pasta_price&folder=pasta" class="edit-good-btn center">Edit</a>
									</div>
								</div>
                        ...
                        echo $out;
                    ?>  
					<a href="add_good.php?category=pasta_price" class="btn-video mt-2 d-block add-good-btn" data-category="pasta_price">Add good</a>
				</div>
			</div>
		</div>
	</div>
