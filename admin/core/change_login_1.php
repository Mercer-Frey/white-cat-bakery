...
$admin_check = selectOneRow($conn, "SELECT login_1, login_2, password, hash_1, hash_2, cook_name_1, cook_name_2 FROM admins");
	if($_COOKIE[$admin_check['cook_name_2']]){
		...
		if($admin_check['login_1'] === $admin_data_check->{'login_1'} 
			...
			if(isset($_POST['new_login_1']) AND isset($_POST['old_login_1'])){
				if(trim($_POST['new_login_1']) !== '' AND trim($_POST['old_login_1']) !==''){
					$new_login_1 = $func->IsLogin($_POST['new_login_1']);
					$old_login_1 = $func->IsLogin($_POST['old_login_1']);
					if($new_login_1 !== false AND $old_login_1 !== false){

						if(md5($_POST['old_login_1']) === $admin_check['login_1']){

							$new_login_1_md = md5($_POST['new_login_1']);

							mysqli_query($conn, "UPDATE admins SET login_1='$new_login_1_md'");
