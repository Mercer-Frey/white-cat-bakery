;(function(){
	...
	document.querySelector('#btn_master_data').onclick = editGallery;
	...
	function editGallery(){
		...
		let image_1 = this.getAttribute('data-image_1-old');
		let image_2 = this.getAttribute('data-image_2-old');
			...
			if(files_1.length>0){
				if(files_1[0].size < 3145728){
					if(valid_extensions.test(files_1[0].name)){
						...
						M.toast({ html: 'Возможные расширения jpg, jpeg, png, gif (картинка 1)'});
						break exit;
					}
				}else{
					M.toast({ html: 'Картинка 1 превышает 3 МБ' });
					break exit;
				}
			}
			...
			if(files_2.length>0){
				if(files_2[0].size < 3145728){
					if(valid_extensions.test(files_2[0].name)){
						...
						M.toast({ html: 'Возможные расширения jpg, jpeg, png, gif (картинка 2)'});
						break exit;
					}
				}else{
					M.toast({ html: 'Картинка 2 превышает 3 МБ' });
					break exit;
				}
			}
